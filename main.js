...
if (!methodCodeQR && !methodCode && !fs.existsSync(`./${authFile}/creds.json`)) {
    do {
    const bluScuro = chalk.hex('#1a1a2e');
    const skyBlue = chalk.hex('#0099ff');
    const viola = chalk.hex('#7c3aed');
    const whiteSoft = chalk.hex('#ECF0F1');
    const rosso = chalk.hex('#ff3333');

        const a = bluScuro('╭━━━━━━━━━━━━━• 𝐙𝐄𝐘𝐍𝐎 𝐂𝐎𝐑𝐄 •━━━━━━━━━━━━━');
    const b = bluScuro('╰━━━━━━━━━━━━━• 𝐙𝐄𝐘𝐍𝐎 𝐄𝐍𝐃 •━━━━━━━━━━━━━');
    const linea = skyBlue('   ─────────◈────────◈─────────◈─────────');
    const sm = viola.bold('   ⚡ SISTEMA DI AUTENTICAZIONE ⚡');

    const qr = viola(' ⌬') + ' ' + chalk.bold.white('MODALITÀ [1]: Sincronizzazione QR');
    const codice = viola(' ⌬') + ' ' + chalk.bold.white('MODALITÀ [2]: Link tramite Codice');

    const istruzioni = [
        viola(' ❯') + whiteSoft.italic(' Inizializzazione protocollo di accesso...'),
        viola(' ❯') + whiteSoft.italic(' Scegli un\'opzione per stabilire il link.'),
        whiteSoft.italic(''),
        bluScuro.italic('                𝐙𝐄𝐘𝐍𝐎 𝐒𝐘𝐒𝐓𝐄𝐌 • 𝐕𝟏.𝟎.𝟎'),
    ];

    const prompt = skyBlue.bold('\n⌬ zeyno-auth ➤ ');
...
${prompt}`);
    ...
    if (!/^[1-2]$/.test(opzione)) {
        console.log(`\n${rosso.bold('✖ ERRORE DI PROTOCOLLO: 𝐙𝐄𝐘𝐍𝐎-𝟒𝟎𝟒')}

${whiteSoft('   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━')}
${rosso.bold('⚠️ Input non riconosciuto dal Core.')} 
${whiteSoft('┌─⭓ Sono validi solo i parametri')} ${chalk.bold.hex('#0099ff')('1')} ${whiteSoft('o')} ${chalk.bold.hex('#0099ff')('2')}
${whiteSoft('└─⭓ Non inserire simboli, spazi o lettere.')}
${skyBlue.italic('\nSupporto Tecnico: Se c\'è un problema constata il creatore del bot, Endy')}
`);
    }
    ...
    if (opzione === '2' || methodCode) {
        ...
        if (!conn.authState.creds.registered) {
            ...
            if (phoneNumber) {
                ...
                phoneNumber = await question(chalk.bgBlack(chalk.bold.hex('#0099ff')(`Inserisci il numero di WhatsApp.\n${chalk.bold.hex('#0099ff')("Esempio: +393471234567")}\n${chalk.bold.hex('#0099ff')('━━► ')}`)));
            ...
            setTimeout(async () => {
                ...
                console.log(chalk.bold.white(chalk.bgHex('#0099ff')('📞 CODICE DI ABBINAMENTO:')), chalk.bold.white(chalk.hex('#7c3aed')(codeBot)));
            }, 3000);
        }
    }
}
